...
- символ # обозначает препятствие, символ . свободное место,
символ C местоположение камеры в лабиринте (камера одна)

Функция, которую нужно написать, принимает как параметр имя этого файла.
Она должна выбрать и вернуть как результат количество свободных точек лабиринта,
попадающих в зону обозрения камеры, причём:
...
fun search(input: String): Int {
    val file = File(input).readLines()
    val row = file.size
    val column = file[0].length
    val array = Array(column) { Array(row) { 0 } }
    var cameraY = 0
    var cameraX = 0
    var count = 0

    for ((numberRow, line) in file.withIndex()) {
    ...
    var x = cameraX
    var y = cameraY
    while (y != 0 && array[x][y - 1] != 2) {  // ↑
        count++
        y--
    }
    x = cameraX
    y = cameraY
    while (y != 0 && x != column - 1 && array[x + 1][y - 1] != 2) { // ↗
        count++
        y--
        x++
    }
    x = cameraX
    y = cameraY
    while (x != column - 1 && array[x + 1][y] != 2) { //  →
        count++
        x++
    }
    x = cameraX
    y = cameraY
    while (y != row - 1 && x != column - 1 && array[x + 1][y + 1] != 2) { //  ↘
        count++
        x++
        y++
    }
    x = cameraX
    y = cameraY
    while (y != row - 1 && array[x][y + 1] != 2) { //  ↓
        count++
        y++
    }
    x = cameraX
    y = cameraY
    while (y != row - 1 && x != 0 && array[x - 1][y + 1] != 2) { //  ↙
        count++
        x--
        y++
    }
    x = cameraX
    y = cameraY
    while (x != 0 && array[x - 1][y] != 2) { // ←
        count++
        x--
    }
    x = cameraX
    y = cameraY
    while (x != 0 && y != 0 && array[x - 1][y - 1] != 2) { // ↖
        count++
        x--
        y--
    }
